...
class DebugODTW:
    def __init__(self, reference_chroma):
        ...
        # Settings
        self.penalty_diagonal = 0.0 
        self.penalty_wait = WAIT_PENALTY    # Deine WAIT_PENALTY
        self.penalty_skip = SKIP_PENALTY    # Deine SKIP_PENALTY
        self.damping = DAMPING_FACTOR        # Dein DAMPING_FACTOR
        self.search_window = SEARCH_WINDOW   # Dein SEARCH_WINDOW

        self.chroma_buffer = deque(maxlen=SMOOTHING_WINDOW) # Dein SMOOTHING

    def step(self, live_chroma_raw):
        # 1. Glättung & Normalisierung
        self.chroma_buffer.append(live_chroma_raw)
        avg_chroma = np.mean(np.array(self.chroma_buffer), axis=0)
        norm = np.linalg.norm(avg_chroma)
        live_vec_norm = avg_chroma / norm if norm > 0.0001 else avg_chroma
        ...
        best_index = self.current_position_index
        best_raw_dist = 1.0 # Default fallback

        for i in range(start_scan, end_scan):
            # A: Lokale Distanz (Cosine)
            dot_prod = np.dot(live_vec_norm, self.ref[:, i])
            dot_prod = max(-1.0, min(1.0, dot_prod))
            local_dist = 1.0 - dot_prod # Das ist der Wert, den wir plotten wollen!
            
            # B: Pfad-Auswahl
            cost_wait = self.accumulated_costs[i] + self.penalty_wait
            cost_step = np.inf
            if i > 0: cost_step = self.accumulated_costs[i-1] + self.penalty_diagonal
            cost_skip = np.inf
            if i > 1: cost_skip = self.accumulated_costs[i-2] + self.penalty_skip
            
            min_prev_cost = min(cost_wait, cost_step, cost_skip)
            
            # C: Gesamtkosten update
            new_costs[i] = local_dist + (min_prev_cost * self.damping)

            # D: Besten Index finden
            if new_costs[i] < best_global_cost:
                best_global_cost = new_costs[i]
                best_index = i
                best_raw_dist = local_dist # <--- DAS SPEICHERN WIR JETZT

        self.accumulated_costs = new_costs
        self.current_position_index = best_index
        
        # RÜCKGABE: Index, Globale Kosten, Lokale Kosten
        return best_index, best_global_cost, best_raw_dist
